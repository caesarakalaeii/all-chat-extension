...
- [~] **TEST-03**: `LlmAgent` helper class — **DROPPED** (replaced by manual Claude MCP scenario scripts in `tests/agent/`; no Stagehand dependency needed)
...
- [~] **TEST-07**: CI documents `ANTHROPIC_API_KEY` — **DROPPED** (no automated agent runner in CI; Claude is the agent via manual MCP sessions)
...
| TEST-03 | Phase 4 | Dropped (intentional) |
| TEST-04 | Phase 4 | Complete |
| TEST-05 | Phase 4 | Complete |
| TEST-06 | Phase 4 | Complete |
| TEST-07 | Phase 4 | Dropped (intentional) |
