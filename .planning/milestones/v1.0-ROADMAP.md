...
- [x] **Phase 3: Kick Platform** - Add full Kick platform support reusing the validated injection base class (completed 2026-03-12)
...
Plans:
- [x] 01-01-PLAN.md — Base class infrastructure: waitForElement(), teardown(), async createInjectionPoint() signature + Wave 0 test scaffolding
- [x] 01-02-PLAN.md — Twitch slot injection: async createInjectionPoint() targeting .chat-shell, scoped MutationObserver, teardown override
- [x] 01-03-PLAN.md — YouTube slot injection: style-tag native chat hide, insertBefore injection, yt-navigate-finish SPA navigation
...
Plans:
- [x] 02-01-PLAN.md — Wave 0 test scaffold: DS-01 through DS-10 skip-stubs in test-design-system.spec.ts
- [x] 02-02-PLAN.md — Build tooling: Tailwind 4 + MiniCssExtractPlugin + postcss migration + tailwind-merge v3
- [x] 02-03-PLAN.md — CSS pipeline: styles.css @theme block + fontsource imports + chat-container.html link tag
- [x] 02-04-PLAN.md — Component migration: ChatContainer header redesign + InfinityLogo + token class rewrite across all 5 components
- [x] 02-05-PLAN.md — ErrorBoundary: class component + ChatContainer wrapper in index.tsx
...
Plans:
- [x] 03-01-PLAN.md — Wave 0 test scaffold: skip-stubs for KICK-01 through KICK-07 + kick-mock.html fixture
- [x] 03-02-PLAN.md — postMessage origin hardening: replace '*' with extensionOrigin in PlatformDetector, twitch.ts, youtube.ts, index.tsx
- [x] 03-03-PLAN.md — KickDetector content script: selector checkpoint + kick.ts implementation (isLiveStream, injection, SPA nav)
- [x] 03-04-PLAN.md — Manifest + webpack wiring: Kick entries in manifest.json and webpack.config.js, build verification
...
Plans:
- [x] 04-01-PLAN.md — Infrastructure: mock WS server, globalSetup/globalTeardown, npm test scripts, playwright.config.ts
- [x] 04-02-PLAN.md — Fast suite implementation: unskip static fs tests, implement page.route() fixture tests, frameLocator assertions
- [x] 04-03-PLAN.md — Agent scenarios + CI job: twitch/youtube/kick scenario markdown files, GitHub Actions test job
...
| 3. Kick Platform | 4/4 | Complete   | 2026-03-12 |
